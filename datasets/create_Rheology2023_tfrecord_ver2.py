...
_INTPUT_TRIPLET_LIST_FILEPATH = flags.DEFINE_string(
    ...
    default='/media/SSD/Frame_Inter_rheology2023/train_text/_3Frame/GLY-all',
...
_OUTPUT_TFRECORD_FILEPATH = flags.DEFINE_string(
    ...
    default='/media/SSD/Frame_Inter_rheology2023/dataset/_3Frame/TFRecord-FILM/train/train',
...
def main(unused_argv):
    ...
    pairs_list = tf.io.gfile.listdir(_INTPUT_TRIPLET_LIST_FILEPATH.value)
    #triplets_list = pd.read_csv(_INTPUT_TRIPLET_LIST_FILEPATH.value) 
    print(f"Data Set : {len(pairs_list)} Folder")

    triplet_dicts = []
    for pairs in pairs_list:
        triplets_list = f"{_INTPUT_TRIPLET_LIST_FILEPATH.value}/{pairs}"
        with open(triplets_list, 'r') as txt:
             sequence_list = [line.strip() for line in txt]
        for i in range(len(sequence_list)): 
            im1, im2, im3 = sequence_list[i].split(' ')
